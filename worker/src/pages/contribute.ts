  ...
  function publicLoad() {
    Promise.all([
      ...
      $("#landed").innerHTML = approved.slice(0, 6).map(function (a) {
        ...
        // An approval that stands and no ring serving it yet: the project's build is on its way into edge — the publish job the approval queued.
        var state = rings.length ? "" : pillHtml("blue", "publishing", "approved; the project's build is on its way into edge");
...
export const FACTORY_COMPONENTS = (_F: Fixture): Component[] => [
  ...
  },
  {
    ...
    reads: [
      { path: "/api/v1/factory/approvals", fields: ["approvals", "approvals.0.standing", "approvals.0.name", "approvals.0.version", "approvals.0.arch", "approvals.0.by", "approvals.0.created_at", "approvals.0.rings"] },
