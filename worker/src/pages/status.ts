...
__CHARTS__
  function renderSystem(d) {
    ...
    var builds = buildsByDay(S, 14), per = {}; builds.series.forEach(function (s) { per[s.name] = s.values; });
    $("#c-builds").innerHTML = bars(builds.labels.map(function (dd, i) { var staged = per.staged[i], published = per.published[i], failed = per.failed[i]; return { label: dd.slice(5), value: staged + published + failed, color: failed > published + staged ? C.red : C.green, title: dd + ": " + staged + " staged, " + published + " published, " + failed + " failed" }; }), function (v) { return v + " build(s)"; });
