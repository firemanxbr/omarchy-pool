...
const SCRIPT = String.raw`
  ...
  // The package's name is its page, at the one address (the shell's pkgHref): with the ring the row is about — the lab for a build nobody decided yet, the most stable ring that serves an approved one, the page's default where it is in none — and the architecture.
  function pkg(name, version, arch, ring) { return '<a href="' + pkgHref(name, ring, arch) + '" title="the package as Packages shows it — where it is, and the factory\'s story of it"><b>' + esc(name) + '</b></a>' + (version ? ' <span class="mono muted">' + esc(version) + '</span>' : ''); }
  // The ring an approval's row is in: the most stable of the rings that serve the package today (the list sorts them lab → stable), none when it is in none yet or any more.
  function ringOf(a) { return a.rings && a.rings.length ? a.rings[a.rings.length - 1] : null; }
  ...
  function renderTiles() {
    ...
    setTiles("#tiles", [
      ["Waiting for review", num(REVIEW.waiting), REVIEW.oldest_ms ? "oldest " + ago(new Date(Date.now() - REVIEW.oldest_ms).toISOString()).replace(" ago", "") : "nothing waiting", REVIEW.waiting ? "warn" : "ok"],
  ...
  function row(cls, name, version, arch, ring, state, line, link) {
    return '<div class="rrow ' + cls + '"><div class="n">' + pkg(name, version, arch, ring) + '</div><span class="pill none">' + esc(arch) + '</span><div class="s">' + state + ' ' + line + '</div>' + (link ? '<a class="go" href="' + esc(link[0]) + '">' + link[1] + '</a>' : '<span></span>') + '</div>';
  }
  ...
  function renderMine() {
    ...
    (blocks.packages || []).filter(function (b) { return isOwner(b.owner); }).forEach(function (b) {
      decided.push(row("act", b.name, null, "all", null, '<span class="pill error">blocked</span>', ago(b.blocked_at) + ' by ' + personLink(b.blocked_by) + ': ' + short(b.blocked_reason, 90), ["/docs/governance", "What a block means →"]));
    });
    ...
    STAGED.filter(function (t) { return isOwner(t.owner); }).sort(function (a, b) { return (b.kind === "project") - (a.kind === "project"); }).forEach(function (t) {
      ...
      if (t.kind === "project") waiting.push(row("", t.name, t.version, t.arch, "lab", '<span class="pill ok">built again</span>', 'the project\'s ' + taskLink(t.id) + ' (from your ' + taskLink(t.from) + ') waits for approval', ["/build/" + t.id, "The build →"]));
      else if (pb && (pb.status === "queued" || pb.status === "leased")) waiting.push(row("", t.name, t.version, t.arch, "lab", '<span class="pill blue">building again</span>', 'the project is building it again (' + taskLink(pb.id) + '), from your ' + taskLink(t.id), ["/build/" + t.id, "Your build →"]));
      else if (pb && pb.status === "failed") waiting.push(row("", t.name, t.version, t.arch, "lab", taskPill("failed", pb.error), 'the project\'s ' + taskLink(pb.id) + ' (from your ' + taskLink(t.id) + ') failed — a maintainer decides', ["/build/" + pb.id, "The project's build →"]));
      else if (t.already) waiting.push(row("", t.name, t.version, t.arch, "lab", '<span class="pill none">already approved</span>', 'your build ' + taskLink(t.id) + ' is of a version approved ' + ago(t.already.at) + ' as ' + taskLink(t.already.task) + ' — nothing to decide', ["/build/" + t.id, "The build →"]));
      else waiting.push(row("", t.name, t.version, t.arch, "lab", taskPill("staged"), 'your build ' + taskLink(t.id) + ' waits for a maintainer' + (t.audit && t.audit.status === "done" && t.audit.verdict ? ' · audit <span class="pill ' + (t.audit.verdict === "ok" ? "ok" : t.audit.verdict === "warn" ? "warn" : "error") + '">' + esc(t.audit.verdict) + '</span>' : t.audit && t.audit.status === "queued" ? ' · audit waiting' : ''), ["/build/" + t.id, "Your build →"]));
    });
    ...
    Object.keys(last).forEach(function (k) {
      ...
      if (a.withdrawn_at) decided.push(row("act", a.name, a.version, a.arch, ringOf(a), taskPill("withdrawn"), 'the approval by ' + personLink(a.by) + ' was withdrawn ' + ago(a.withdrawn_at) + ' by ' + personLink(a.withdrawn_by) + ': ' + short(a.withdrawn_reason, 100) + ' — another maintainer decides', ["/build/" + a.task_id, "The build →"]));
      else if (a.decision === "rejected") decided.push(row("act", a.name, a.version, a.arch, ringOf(a), taskPill("rejected"), ago(a.created_at) + ' by ' + personLink(a.by) + ': ' + short(a.note, 110), ["/factory", "Fix it, build again →"]));
      else { var inRings = a.rings && a.rings.length ? a.rings : (p && p.status === "published" ? ["edge"] : []); decided.push(row("ok", a.name, a.version, a.arch, inRings[inRings.length - 1], taskPill("approved"), ago(a.created_at) + ' by ' + personLink(a.by) + (inRings.length ? ' — in ' + inRings.join(" · ") + ', signed by the pool' : ' — the project\'s build is on its way into edge') + (a.note ? ' · ' + short(a.note, 80) : ''), inRings.length ? [pkgHref(a.name, inRings[inRings.length - 1], a.arch), "The package →"] : ["/build/" + a.task_id, "The build →"])); }
    });
  ...
  function renderStaged() {
    ...
    pager("#staged", rows, function (t) {
      ...
      return '<tr id="t-' + t.id + '"' + (project ? ' class="project-row"' : '') + (forYou ? ' class="for-you"' : mine ? ' class="mine-row"' : '') + '><td>' + pkg(t.name, t.version, t.arch, "lab") + (det.license ? ' <span class="dim">' + esc(det.license) + '</span>' : '') + (t.url ? ' <a class="run dim" href="' + esc(t.url) + '" title="' + esc(t.url) + '">source</a>' : '') + '<br>' + category(t) + '</td><td>' + esc(t.arch) + '</td>' +
...
export const REVIEW_COMPONENTS = (F: Fixture): Component[] => [
  ...
  },
  {
    ...
    script: ['$("#mine-waiting")', "Nothing of yours waiting", "Nothing of yours here", 'href="/request"', "t.project_build", "t.already.task", ">built again<", "t.audit.verdict", 't.version, t.arch, "lab"'],
  ...
  },
  {
    ...
    script: ['$("#mine-decided")', "No decision on a package of yours yet", "once you are signed in", "MINE.packages", "a.withdrawn_at", "a.rings", "ringOf(a)", "pkgHref(a.name, inRings[inRings.length - 1], a.arch)", "blocks.packages"],
    reads: [
  ...
  },
  {
    ...
    script: ['pager("#staged"', 'pkg(t.name, t.version, t.arch, "lab")', "function pkg(name, version, arch, ring)", "pkgHref(name, ring, arch)", "gatePill(t.vet, t.evidence.tests)", "auditPill(t.audit, t.evidence.audit)", "trialPill(t.trial, t.evidence.trial)", "t.built_by", "sc.projected", '"project-row"', '"mine-row"'],
    reads: [
      {
  ...
  },
  {
    ...
    script: ['pager("#decisions"', "pkg(a.name, a.version, a.arch, ringOf(a))", "function ringOf(a)", "a.standing", "a.rebuild_task", "a.rebuild_status", "a.rebuild_result", "a.withdrawn_reason"],
