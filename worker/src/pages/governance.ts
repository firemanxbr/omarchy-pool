...
const BODY = String.raw`
  ...
  <section id="categories">
    ...
    <p class="sub">There are no groups: no package belongs to an area whose maintainers own it, and no maintainer reviews only a part of the pool. What a package <em>is about</em> — for a person browsing — is its <b>category</b>, one of a fixed list: ${CATEGORIES.map((c) => `<code>${c}</code>`).join(", ")}.</p>
    <div class="steps">
      ...
      <div class="step"><h3>It travels with the package</h3><p>The registry, the package page (<em>who stands behind it</em>), the profile's package list and the seal carry it. It says where to look, never who may approve. The repository holds no package recipes: every package is a request on the record, built by the pool — since 2026-09-17 nothing about a package goes through a pull request.</p></div>
    </div>
  </section>
  ...
  <section id="workers">
    ...
    <div class="steps">
      ...
      <div class="step"><h3>Yours — or everyone's, if you say so</h3><p>A registered worker builds <b>its owner's packages</b>; started with <code>WORKER_SHARED=1</code> it also takes the shared queue — every request lands there the moment its record is written, and the best idle shared worker of the architecture builds it first. Sharing is the owner's word alone: their machine, their agent's key, strangers' recipes in a fresh container each time, evidence only — nothing a shared worker builds reaches the pool without the project's own build and a maintainer's decision. Nobody's laptop is shared by accident.</p></div>
...
const SCRIPT = String.raw`
  var ROLES = { contributor: ["Contributor", [["register any package, no permission needed", "have every request built in the shared queue, the best idle shared worker first — a worker of your own takes yours at once, and shared it helps everyone", "get every build as evidence, publicly", "a public profile and a score"], ["ship bytes to users directly", "approve anything, including their own"]]], maintainer: ["Maintainer", [["read a staged build with the evidence in front of them, and reject it or have the project build it again — never their own", "approve or reject the project's build — the approval is the decision, on the record with a name", "settle each package's category", "vouch for a worker as a project worker — with a second maintainer, never the owner", "block a contributor or a package, with the reason on the record — another maintainer lifts it", "roll a ring back, and run any pipeline step by hand", "review and merge pull requests — never their own; propose a new maintainer"], ["use anything a contributor built — not the package, not the PKGBUILD: the project builds what its agent wrote", "decide on their own package — approve, reject or have the project build it — even as the only maintainer (withdrawing an approval is undoing, not deciding)", "be named anywhere but factory/MAINTAINERS.toml"]]], workers: ["The project's workers", [["claim tasks of their role and architecture with a lease", "run the pool's jobs, the audit of a staged build, the project's builds of reviewed packages and of the recipes on main", "write the audit when their owner set an agent key"], ["build from a contributor's staged artifact", "pull a new package that has no evidence and no review yet", "decide anything — the audit is evidence, never a verdict", "hold the pool's signing key: signing happens in the brain"]]] };
...
export const GOVERNANCE_COMPONENTS = (_F: Fixture): Component[] => {
  ...
  return [
    ...
    {
      // The list is CATEGORIES itself, rendered in the prose; the anchors read it back in the same order.
      id: "governance.categories",
      page,
      anchor: ['id="categories"', "<h2>Categories, not groups</h2>", `one of a fixed list: ${CATEGORIES.map((c) => `<code>${c}</code>`).join(", ")}.`, "<h3>The project's agent proposes it</h3>", "<h3>A maintainer settles it</h3>", "<h3>It travels with the package</h3>"],
      visible: EVERYONE,
    },
    {
