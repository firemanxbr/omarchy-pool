...
docker compose up -d           # podman compose works the same</pre>
      ...
      <div class="step"><h3>2. Give it work</h3><p><a href="/request">Request a package</a> (the project's URL, a description, the licence, the checklist). The build starts by itself, in the shared queue: the best idle shared worker of the architecture takes it — a worker of your own at once, shared or not. <b>Build</b> on your page names a worker, sends a queued build back to the shared queue or takes it out, and runs it again. The <em>Builds</em> table follows it, and the <em>Workers</em> table shows it alive. When the build is staged, a maintainer sees it on <a href="/review">Review</a>.</p></div>
...
export const DOCS_WORKERS_COMPONENTS = (F: Fixture): Component[] => [
  ...
  },
  {
    ...
    page: "/docs/workers",
    anchor: ["<h3>2. Give it work</h3>", '<a href="/request">Request a package</a>', "The build starts by itself, in the shared queue", "a worker of your own at once, shared or not", "<b>Build</b> on your page names a worker", '<a href="/review">Review</a>'],
    visible: EVERYONE,
  },
  {
