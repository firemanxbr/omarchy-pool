...
const FACTORY_READ: Row[] = [
  { routes: ["GET /factory?limit="], text: "Workers the pool has heard from (owner, trust, mode, the agent each reported, current task), the queue (every kind: builds, pool jobs, audits), package requests, counts." },
  ...
  { routes: ["GET /factory/approvals", "GET /factory/maintainers", "GET /factory/trust", "GET /factory/blocks"], text: "The record: every decision with who signed it and whether it stands (<code>standing</code>: approved, not withdrawn); the maintainers (from <code>factory/MAINTAINERS.toml</code>, with since when); project-trusted workers; what is blocked now and why." },
...
const WRITE_JOBS: Row[] = [
  ...
  { routes: ["PUT /security/advisories", "PUT /security/matches", "POST /security/prune?before="], text: "The security job's writes: the advisories it read from the feeds, what they match in the rings, and the prune of what is older than a date." },
  { routes: ["POST /events", "POST /pool/gc", "POST /pool/relayout"], text: "Record a journal entry; run retention; one step of the one-time move to one directory per source (the <code>relayout</code> job)." },
  ...
  return [
    ...
    },
    {
      ...
      reads: [
        ...
        { path: "/api/v1/factory/approvals", fields: ["approvals", "approvals.0.id", "approvals.0.task_id", "approvals.0.name", "approvals.0.decision", "approvals.0.by", "approvals.0.standing", "approvals.0.rings"] },
