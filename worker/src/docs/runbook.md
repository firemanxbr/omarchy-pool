...
- **New upstream versions** — two paths, one rule (evidence before review):
  - a package a contributor registered: once a day (05:45 UTC) the brain
    ...
    request whose author has no worker for the architecture goes to the
    shared workers at once; a build asked for one worker — `worker` in
    `POST /factory/packages/<name>/build`, `pinned_to` on the task — waits
    for that worker only). A
