...
| metrics snapshot (`src/metrics.ts`) | every 30 minutes | taken by the Worker itself, no job: the pool's jobs of the last 7 days (runs, failures, worker minutes, per kind), builds, workers alive, pool totals and ring sizes, as a `metrics` event — the pool's history (the growth chart, and the ring sizes the Pool page draws); the jobs of the week the Status page's tiles, table and charts say are one reduce over the live series (`jobs_daily`, the shell's `jobsSummary`), never the snapshot's, which is up to half an hour older and counts a cancelled job as a success. Between two changes of the pool a snapshot carries the previous one's pool block and counts its jobs afresh |
