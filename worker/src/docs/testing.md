...
| `lists.test.ts` | lists rendered from the code that owns them, never copied: the request's four confirmations, the categories, the sources table, the pacman.conf sample, the cost cadence and the budget's lines, the journal's kinds and the job kinds on the API page, the API page's rows against the routes the router serves; the rings, the architectures and the severities (`meta.ts`) spliced into the shell once — `PROMISED_RINGS`, `RINGS_UPWARD`, `ARCHES`, `SEVERITIES` — and every page script reading them, none typing a list of its own, the manifests pinning the read; the Workers page's pool kinds as `JOB_KINDS`; a worker alive by one number, `WORKER_ALIVE_MINUTES`, in the listing, a person's page, the snapshot, the scheduler and the shell's titles |
